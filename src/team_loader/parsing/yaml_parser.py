...
from src.type_defs import JsonArray, JsonObject, JsonValue
...
class YamlParser:
    ...
    def parse(self, text: str) -> JsonValue:
    ...
    def _parse_block(self, index: int, indent: int) -> tuple[JsonValue, int]:
        index = self._skip_blank(index)
        if index >= len(self._lines):
            return {}, index
    ...
    def _parse_mapping(self, index: int, indent: int) -> tuple[JsonObject, int]:
        result: JsonObject = {}
    ...
    def _parse_list(self, index: int, indent: int) -> tuple[JsonArray, int]:
        result: JsonArray = []
        while index < len(self._lines):
            index = self._skip_blank(index)
            if index >= len(self._lines) or self._indent_of(index) < indent:
            ...
            if item_text == "":
                ...
                item_dict: JsonObject = {key: self._parse_scalar(value) if value else None}
                next_index = self._skip_blank(index + 1)
