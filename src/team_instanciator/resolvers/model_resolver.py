...
class ModelResolver:
    ...
    def resolve(self, team: TeamDefinition, agent: AgentDefinition) -> object:
        ...
        policy = ModelReliabilityPolicy.from_configuration(self._configuration)
        kwargs = self._configuration.model_kwargs(model)
        kwargs.update({key: value for key, value in reliability_init_kwargs(model, policy).items() if key not in kwargs})
        ...
        resolved = init_chat_model(model=model, **kwargs)
        return wrap_model_for_reliability(resolved, model_name=model, policy=policy)
