...
Ce document garde les détails techniques qui étaient auparavant dans le README.
Le README racine reste volontairement court et orienté usage.
...
The Web UI reads the same SQLite checkpoint database as the CLI:
...
- `GET /api/state` exposes persistent agents and `tools:*` run summaries.
- `GET /api/task-run?thread_id=...&run_id=tools:...` loads a complete
  non-persistent run transcript on demand.

See [webui/README.md](webui/README.md) for the short Web UI reference.
...
- Do not claim production readiness from local validation alone; use the CI
  results and workflow artifacts as release evidence.
